...
<div class="space-y-6">
	...
	<!-- Colonne principale -->
	<div class="min-w-0 transition-all duration-300 {selectedReport ? 'lg:w-96 lg:shrink-0' : 'flex-1'} overflow-hidden space-y-6">
		<!-- Tableau -->
		<div class="overflow-hidden rounded-lg border border-gray-200 bg-white">
			<table class="w-full text-sm">
				<thead class="bg-gray-50 text-xs font-medium uppercase tracking-wide text-gray-500">
					<tr>
						<th class="px-4 py-3 text-left text-gray-400">Q#</th>
				...
				<tbody class="divide-y divide-gray-100">
					{#each data.rows as report}
						...
						>
							<td class="px-4 py-3 text-gray-400 tabular-nums">{report.questionId}</td>
							<td class="px-4 py-3">
					...
					{:else}
						<tr>
							<td colspan={selectedReport ? 3 : 6} class="px-4 py-8 text-center text-sm text-gray-400">
								Aucun signalement.
							</td>
						</tr>
					{/each}
